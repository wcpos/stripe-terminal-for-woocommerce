...
/**
 * Isolate optional Pro and overloaded transport classes.
 *
...
class PosReaderSettingsTest extends ServerTestCase {
	...
	protected function setUp(): void {
		...
		if ( ! defined( 'MINUTE_IN_SECONDS' ) ) {
			// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals -- WordPress constant stub.
	...
	/** Create a gateway without constructor side effects. */
	 ...
	 * Stub Pro support.
	 *
	 * @param bool $supported Whether Pro supports the server contract.
	 */
	...
	/** Fields reuse provider projection and cache for five minutes. */
	public function test_fields_reuse_provider_projection_and_cache_for_five_minutes(): void {
		...
		Functions\when( 'set_transient' )->alias(
			function ( $key, $value, $ttl ) use ( &$cache, &$cache_writes ) {
				$cache[ $key ] = $value;
				$cache_writes[] = array( $value, $ttl );
				return true; }
		);
	...
	/**
	 * Preserve reader choices when discovery is unavailable.
	 *
	...
	public function test_unavailable_list_uses_text_and_omits_multiselect( string $context ): void {
		...
		if ( 'key' === $context ) {
			unset( $this->options[ self::GATEWAY_OPTION ]['test_secret_key'] ); }
	...
	public function unavailable_contexts(): array {
		return array_map(
			...
			array( 'error', 'section', 'frontend', 'ajax', 'key' )
		);
	}

	/** Save mirrors saved values and replaces shrunk lists. */
	public function test_save_mirrors_saved_values_and_replaces_shrunk_lists(): void {
		$this->pro();
		$this->options[ self::FREE_OPTION ] = array(
			'gateways' => array( 'cash' => array( 'enabled' => true ) ),
			'default' => 'cash',
		);
		$gateway = $this->gateway();
		$gateway->init_form_fields();
		...
		$_POST[ $gateway->get_field_key( 'allowed_readers' ) ] = array(
			2 => 'tmr_a',
			4 => '',
			6 => 'tmr_b',
			7 => false,
		...
		$this->assertSame(
			array(
				'gateways' => array(
					'cash' => array( 'enabled' => true ),
					Settings::GATEWAY_ID => array(
						'default_reader' => 'tmr_a',
						'allowed_readers' => array( 'tmr_a', 'tmr_b' ),
		...
		$this->assertSame(
			array(
				'default_reader' => '',
				'allowed_readers' => array( 'tmr_b' ),
				'lock_to_default' => false,
			),
			$this->options[ self::FREE_OPTION ]['gateways'][ Settings::GATEWAY_ID ]
		);
	}

	/** Failed fetch preserves saved values on save. */
	...
	/** Without pro only gateway settings are written. */
	...
	/** Migration seeds once without overwriting existing settings. */
	public function test_migration_seeds_once_without_overwriting_existing_settings(): void {
		$this->pro();
		$this->options[ self::GATEWAY_OPTION ] += array(
			'default_reader' => 'tmr_a',
			'allowed_readers' => array( 'tmr_a' ),
			'lock_to_default' => 'yes',
		);
		$this->options[ self::FREE_OPTION ] = array(
			'gateways' => array(
				Settings::GATEWAY_ID => array(
					'allowed_readers' => array( 'tmr_existing' ),
					'enabled' => true,
				),
			),
		);
		...
		Functions\when( 'update_option' )->alias(
			function () {
				$this->fail( 'Migration must not write twice.' );
			}
		);
		Pos_Reader_Settings::migrate_once();
		$this->assertSame( $before, $this->options );
